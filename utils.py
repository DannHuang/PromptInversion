...
def preprocess(max_height=256, max_width=256):
    ...
    for k,v in attn_maps.items():
        v = torch.mean(v.detach().cpu(),axis=0).squeeze(0)
        _, h, w = v.shape
...
def visualize_and_save_attn_map(attn_map, tokenizer, prompt, postfix=''):
    # match with tokens
    ...
    for i, (token, token_attn_map) in enumerate(zip(tokens, attn_map)):
        ...
        token_attn_map = token_attn_map.detach().cpu().numpy()
